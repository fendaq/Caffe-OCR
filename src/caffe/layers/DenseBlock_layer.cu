...
namespace caffe {
	...
	void DenseBlockLayer<Dtype>::GPU_Initialization() {
		...
		if (useBC) {
			...
			CUDNN_CHECK(cudnnSetConvolution2dDescriptor_v5(*convBC_Descriptor, 0, 0, 1, 1, 1, 1, CUDNN_CONVOLUTION, cudnn::dataType<Dtype>::type));
		}
		...
		CUDNN_CHECK(cudnnSetConvolution2dDescriptor_v5(*this->conv_Descriptor, 1, 1, 1, 1, 1, 1, CUDNN_CONVOLUTION, cudnn::dataType<Dtype>::type));
